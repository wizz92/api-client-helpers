...
class CurlRequest 
{
    public static function apiRequestProxy(Request $request)
    {
        ...
        if (in_array($method, ["PUT", "POST", "DELETE"]))
        {
            ...
            {
                $data['files'] = self::prepare_files_for_curl($data);
            }
    ...
    public static function getFilenameFromHeader($contentDisposition)
    {
    ...
    // TODO seems to be useless
    public static function getPathFromHeaderOrRoute($contentDisposition, $slug)
    {
        if ($contentDisposition) {
            preg_match('/filename="(.*)"/', $contentDisposition, $filename);
            $filename = clear_string_from_shit($filename[1]);
    ...
    public static function prepare_files_for_curl(array $data, $file_field = 'files')
    {
